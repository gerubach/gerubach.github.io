...
def temperature_coefficient_plot(country, year_begin, year_end, month, min_obs, **kwargs):
    ...
    plot_title = f"""Estimates of yearly temperature increase in 
                    {months[month]} for stations in {country}, 
                    years {year_begin} to {year_end}"""
...
def global_temperatures_plot(year_begin, year_end, month, min_obs):
    months = {1 : "January", 2 : "February", 3 : "March", 
            ...
            11 : "November", 12 : "December"}
    plot_title = f"""Estimates of yearly temperature increase in 
                    {months[month]},{'<br>'}years {year_begin} 
                    to {year_end}"""
...
def faceted_hist(country, years, months):
    ...
    months.sort()
    years.sort()
    plot_title = f"""Distribution of temperatures in {country} 
                    for {'<br>'}selected years and months"""
    junk = query_climate_database3(country, years, months)
    junk["Month"] = junk["Month"].apply(lambda x: month_conversion[x])
    fig = px.histogram(junk, x="Temp", facet_row = "Year", 
